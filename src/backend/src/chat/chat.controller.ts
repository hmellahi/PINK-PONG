...
export class ChatController {
  ...
  @Post('addMember')
  @HttpCode(200)
  async addMember(@Req() request: RequestWithUser, @Body() data: AddMemberDto) {
    const { user } = request;
  ...
  @Post('addAdmin')
  @HttpCode(200)
  async addAdmin(@Req() request: RequestWithUser, @Body() data: AddAdminDto) {
    const { user } = request;
