...
export class ChatService {
  ...
  async createChannel(data: CreateChannelDto) {
    if (data.isLocked) data.password = await bcrypt.hash(data.password, 10);
  ...
  async getChannels(user: UserEntity) {
    return (
      ...
      if (!this.isMember(channel, user)) return channel;
  ...
  async joinChannel(data: JoinChannelDto) {
    ...
    if (!channel || this.isMember(channel, data.user))
      throw new HttpException(
        'Channel not found or You already Joined',
  ...
  async addMember(member: UserEntity, data: AddMemberDto) {
    ...
    if (!this.isMember(channel, member)) throw new ForbiddenException();
  ...
  async leaveChannel(member: UserEntity, data: LeaveChannelDto) {
    ...
    if (!channel || !this.isMember(channel, member))
      throw new HttpException(
        'Channel not found or User not A member',
        HttpStatus.BAD_REQUEST,
      );
    ...
    await this.channelRepository.update(
      { id: data.channelId, owner: member },
      { owner: null },
    );
  }

  public async updateChannelPassword(
    member: UserEntity,
    data: UpdateChannelPassword,
  ) {
    const channel = await this.channelRepository.findOne(
      { id: data.channelId },
    ...
    if (
      (!channel.owner || channel.owner.id !== member.id) &&
      !this.isAdmin(channel, member)
    )
      throw new UnauthorizedException();
    if (data.isLocked) channel.password = await bcrypt.hash(data.password, 10);
  ...
  public async addAdmin(member: UserEntity, data: AddAdminDto) {
    ...
    if (!channel || this.isAdmin(channel, newAdmin))
      throw new HttpException(
        'Channel not exist  or User already admin',
        HttpStatus.BAD_REQUEST,
      );
    if (!this.isAdmin(channel, member)) throw new ForbiddenException();
    await this.channelRepository
      .createQueryBuilder()
      .relation('admins')
      .of({ id: data.channelId })
